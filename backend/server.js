...
app.post('/api/invoke-agent', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Use full ARN for AgentCore API
    const region = 'us-west-2'; // Force us-west-2 for AgentCore
    
    // DEBUG: Log what we're using
    console.log(`[DEBUG] Full Agent ARN: ${agentRuntimeArn}`);
    console.log(`[DEBUG] Region forced to: ${region}`);
    console.log(`[DEBUG] API Host: bedrock-agentcore.${region}.amazonaws.com`);
    console.log(`[DEBUG] API Path: /runtimes/${encodeURIComponent(agentRuntimeArn)}/invocations`);
